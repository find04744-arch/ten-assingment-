...
@section('page_subtitle')
    Update product or item details
@endsection
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3>Edit Product Item Form</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.product-items.update', $product_item) }}" method="post"
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category" class="form-select @error('category') is-invalid @enderror"
                                    ...
                                    @foreach ($categories as $cat)
                                        <option value="{{ $cat }}"
                                            {{ old('category', $product_item->category) === $cat ? 'selected' : '' }}>
                                            {{ ucfirst($cat) }}</option>
                                    @endforeach
                                </select>
                                @error('category')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="item_type" class="form-select @error('item_type') is-invalid @enderror"
                                    ...
                                    <option value="feature"
                                        {{ old('item_type', $product_item->item_type) == 'feature' ? 'selected' : '' }}>
                                        Feature Item (Icon + Text + Image)</option>
                                    <option value="gallery"
                                        {{ old('item_type', $product_item->item_type) == 'gallery' ? 'selected' : '' }}>
                                        Gallery Image (Image only)</option>
                                </select>
                                @error('item_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3" id="subcategory-field">
                                ...
                                <input type="text" name="subcategory"
                                    ...
                                    value="{{ old('subcategory', $product_item->subcategory) }}" list="subcategory-options"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="title"
                                    ...
                                    value="{{ old('title', $product_item->title) }}" required>
                                @error('title')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="text" name="icon"
                                    ...
                                    value="{{ old('icon', $product_item->icon) }}" placeholder="e.g. fa fa-user">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $product_item->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                @if ($product_item->image_path)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/' . $product_item->image_path) }}" alt="Current Image"
                                            style="height: 50px; border-radius: 4px;">
                                    </div>
                                @endif
                                <input type="file" name="image_path"
                                    class="form-control @error('image_path') is-invalid @enderror" accept="image/*">
                                <div class="form-text">Recommended dimensions: 600x445 px. Leave empty to keep current
                                    image. Maximum file size: 300KB (200–300KB recommended).</div>
                                @error('image_path')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
