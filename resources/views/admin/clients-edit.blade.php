...
@section('subtitle', 'Update client details')
...
<div class="fade-up" style="max-width:760px">
    ...
    <div style="border-radius:22px;overflow:hidden;box-shadow:0 4px 28px rgba(0,0,0,.08);border:1px solid #e8ecf4">
        <div style="background:linear-gradient(135deg,#431407,#7c2d12,#c2410c);padding:20px 26px;display:flex;align-items:center;justify-content:space-between;position:relative;overflow:hidden">
            ...
            <div style="display:flex;align-items:center;gap:14px">
                ...
                <div>
                    <p style="font-size:16px;font-weight:800;color:#fff">Edit — {{ $client->name }}</p>
                    <p style="font-size:12px;color:rgba(254,215,170,.55);margin-top:2px">ID #{{ $client->id }}</p>
                </div>
            </div>
            @if($client->logo_path)
                <img src="{{ Storage::url($client->logo_path) }}" alt="{{ $client->name }}" style="width:48px;height:48px;border-radius:10px;object-fit:cover;border:2px solid rgba(255,255,255,.2);position:relative">
            @endif
        </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid-2">
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" name="website_url" value="{{ old('website_url',$client->website_url) }}" class="form-input">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid-2">
                        <div class="form-group">
                            ...
                            @if($client->logo_path)
                                <img src="{{ Storage::url($client->logo_path) }}" alt="Current" class="img-preview" style="margin-bottom:8px">
                            @endif
                            <input type="file" name="logo" class="form-input" accept="image/*" style="padding:7px 14px">
                            <p class="form-hint">Leave blank to keep current logo</p>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Status <span>*</span></label>
                            <select name="status" class="form-input form-select" required>
                                <option value="1" {{ old('status',$client->status ? '1' : '0') === '1' ? 'selected' : '' }}>Active — visible on site</option>
                                <option value="0" {{ old('status',$client->status ? '1' : '0') === '0' ? 'selected' : '' }}>Inactive — hidden</option>
                            </select>
                        </div>
                    </div>
                </div>
